...
function html5AddSession($db, $name, $type, $port, $userid, $hostname = null, $servicePort = null, $username = null, $password = null, $onresize = null,$template = null)
{
	...
	$statement->execute(['connection_id'=>$connectionId]);
	 if ($type == 'rdp-tls' ) {
	 	$query = "replace into guacamole_connection ( connection_id , connection_name , protocol ) values ( " . $connectionId . ",'" . $name . "','" . 'rdp' . "');";
	 }
	 else {
	...
	if($type == 'rdp-tls'){
			...
			$connectionData = [
				...
				if ($password != null && $username != null) {
							...
							$connectionData[] = "( " . $connectionId . ",'resize-method', 'display-update' )";		
				}
				else{
	 ...
	 if($type == 'spice'){
			...
			$connectionData[] = "( " . $connectionId . ", 'hostname', '".$hostname."' )";
			$connectionData[] = "( " . $connectionId . ", 'port', '".$servicePort."' )";
			$connectionData[] = "( " . $connectionId . ",'file-transfer-create-folder','true' )";
			$connectionData[] = "( " . $connectionId . ",'file-transfer','true' )";
			$connectionData[] = "( " . $connectionId . ",'file-directory', '/tmp/" . $connectionId ."' )"; 
			$connectionData[] = "( " . $connectionId . ",'enable-audio','true' )";
			$connectionData[] = "( " . $connectionId . ",'enable-audio-input','true' )";
			$connectionData[] = "( " . $connectionId . ",'server-layout','fr_fr_azerty' )";    
	}

	if($type == 'telnet' || $type =='ssh') {
	 		$connectionData[] = "( " . $connectionId . ", 'hostname', '".$hostname."' )";
			$connectionData[] = "( " . $connectionId . ", 'port', '".$servicePort."' )";
			$connectionData[] = "( " . $connectionId . ",'color-scheme','green-black' )";
			$connectionData[] = "( " . $connectionId . ",'font-size','12' )";
		    $connectionData[] = "( " . $connectionId . ",'enable-sftp','true' )";
		    $connectionData[] = "( " . $connectionId . ",'sftp-root-directory','/tmp/' )"; 
...
function addWiresharkSystem($lab, $node_id, $interface_id)
{
	...
	$cmd = 'sudo ip link | grep rdp' . $uniqueId;
	$o = [];
	exec($cmd, $o, $rc);

	if (count($o) == 0) {
		$cmd = 'sudo ip link add rdp' . $uniqueId . ' type veth peer name dc0' . $uniqueId;
		exec($cmd, $o, $rc);
		$cmd = 'sudo ip link set dev rdp' . $uniqueId . ' up';
		exec($cmd, $o, $rc);

		$cmd = 'sudo ip link set dev dc0' . $uniqueId . ' up';
		exec($cmd, $o, $rc);

		// Add eth1 for docker

		$cmd = 'sudo ip link set netns ' . $pid . ' dc0' . $uniqueId . ' name eth1 address ' . '48:' . sprintf('%02x', $lab_session) . ':' . sprintf('%02x', $node_id / 512) . ':' . sprintf('%02x', $node_id % 512) . ':' . sprintf('%02x', $interface_id) . ':' . sprintf('%02x', 1) . ' up';
		exec($cmd, $o, $rc);
	...
	$cmd = 'sudo ip link | grep span' . $uniqueId;
	$o = [];
	exec($cmd, $o, $rc);

	if (count($o) == 0) {

		$cmd = 'sudo ip link add span' . $uniqueId . ' type veth peer name cap' . $uniqueId;
		exec($cmd, $o, $rc);

		$cmd = 'sudo ip link set dev span' . $uniqueId . ' up';
		exec($cmd, $o, $rc);

		$cmd = 'sudo ip link set dev span' . $uniqueId. ' mtu 9000';
		exec($cmd, $o, $rc);

		$cmd = 'sudo ip link set dev cap' . $uniqueId . ' up';
		exec($cmd, $o, $rc);

		$cmd = 'sudo ip link set dev cap' . $uniqueId. ' mtu 9000';
		exec($cmd, $o, $rc);

		// add capture port to docker 
		$cmd = 'sudo ip link set netns ' . $pid . ' cap' . $uniqueId . ' name eth0 address ' . '48:' . sprintf('%02x', $lab_session) . ':' . sprintf('%02x', $node_id / 512) . ':' . sprintf('%02x', $node_id % 512) . ':' . sprintf('%02x', $interface_id) . ':' . sprintf('%02x', 0) . ' up';
		exec($cmd, $o, $rc);
...
function deleteWiresharkSystem($tenant, $lab_session, $node_id, $interface_id, $port, $ipAddress)
{
	...
	$cmd = 'sudo ip link delete rdp' . $uniqueId;
	$o = [];
	exec($cmd, $o, $rc);

	// remove span connection to eth1
	$cmd = 'sudo ip link delete span' . $uniqueId;
	$o = [];
	exec($cmd, $o, $rc);
...
function getTemplates()
{
	...
	foreach ($templates as $templ => $desc) {
		try {
			...
			if (!is_file(BASE_DIR . '/html/'. TPL_DIR .'/' . $templ . '.yml')) {
