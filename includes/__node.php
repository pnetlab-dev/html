	...
	public function __construct($p, $id, $tenant, $lab)
	{
		...
		try {
			...
			if (is_file('/opt/unetlab/html/devices/' . $this->type . '/device_' . $this->template . '.php')) {
				...
				if ( $this->template == 'timos-ng') { 

					require_once('/opt/unetlab/html/devices/' . $this->type . '/device_timos.php');
					$class = 'device_timos';
					$this->deviceFactory = new $class($this);
				} 
			    else if ( $this->template == 'timosiom-ng') { 

					require_once('/opt/unetlab/html/devices/' . $this->type . '/timosiom.php');
					$class = 'timosiom';
					$this->deviceFactory = new $class($this);
				} 
				else if ( $this->template == 'timoscpm-ng') { 

					require_once('/opt/unetlab/html/devices/' . $this->type . '/timoscpm.php');
					$class = 'timoscpm';
					$this->deviceFactory = new $class($this);
				} 
			}
