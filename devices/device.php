...
class device
{
    ...
    function __construct($node)
    {
        $this->node = $node;
        try {
            $this->tpl = yaml_parse_file(BASE_DIR . '/html/'. TPL_DIR .'/' . $this->getTemplate() . '.yml');
        } catch (Exception $e) {
        ...
        if ($html5 != 1) {
            switch ($this->console) {
                ...
                case 'rdp-tls':
                    ...
                    break;;
                case 'spice':
                    return 'spice://' . $_SERVER['SERVER_NAME'] . ':' . $this->getPort();
                    break;;
                case 'winbox':
        ...
        if ($html5 != 1) {
            switch ($this->console_2nd) {
                ...
                case 'rdp-tls':
                    ...
                    break;;
                case 'spice':
                    return 'spice://' . $_SERVER['SERVER_NAME'] . ':' . $secondPort;
                    break;;
                case 'winbox':
    ...
    public function getGuacConsoleLink($index)
    {
        ...
        if ($console == 'rdp' || $console == 'rdp-tls' || $console == 'vnc' || $console == 'spice' ) {
            html5AddSession($html5_db, $this->name . '_' . $this->getId() . '_' . $username, $console, $port, $this->getTenant(), null, null, $this->username, $this->password, 'reconnect',$this->getTemplate());
        } else {
    ...
    public function start()
    {
        ...
        $cmd = $this->command();
         ...
         if ( $this->type == 'qemu' ) { // check vm / bare for vmx passthrough
                if ( HYPERVISOR == "vm" ) {
...
#------------------------------------------------------------------------------------------------------------------------------------------------------#
            $cmd = 'sudo ip link | grep vunl' . $this->getSession() . '_ | sed \'s/.*\(vunl[0-9]\+_[0-9]\+\).*/\1/g\' | while read line; do sudo ip link set $line down; sudo tunctl -d $line; sudo ip link delete $line ; done'; 
            //error_log(date('M d H:i:s ') . 'ERROR: ' . $cmd);
            exec($cmd, $o, $rc);
#------------------------------------------------------------------------------------------------------------------------------------------------------#
               usleep(200000); //sleep waiting for vnet free
             $cmd = 'sudo ip link  show | grep vnet |  grep DOWN | sed \'s/.*\(vnet[0-9]\+_[0-9]\+\).*/\1/g\' |  while read line; do sudo ifconfig $line down; sudo brctl delbr $line; done';
           // error_log(date('M d H:i:s ') . 'ERROR: ' . $cmd);
            exec($cmd, $o, $rc);
