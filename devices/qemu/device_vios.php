...
class device_vios extends device_qemu
{
    ...
    public function prepare()
    {
        ...
        else  if (!is_file($this->getRunningPath() . '/vios_final_startup-config.txt') && !is_file($this->getRunningPath() . '/.configured' )) {
            copy('/opt/unetlab/startup_configs/vios/vios_base_startup-config.txt', $this->getRunningPath() . '/vios_base_startup-config.txt');
            $startup_vios = $this->getRunningPath() . '/vios_base_startup-config.txt' ;    
            $file_contents = file_get_contents($startup_vios);
            $file_contents = str_replace("hostname", "hostname  $this->name  ", $file_contents);
            file_put_contents($this->getRunningPath() . '/vios_base_startup-config.txt',  $file_contents);
            copy($this->getRunningPath() . '/vios_base_startup-config.txt', $this->getRunningPath() . '/vios_final_startup-config.txt' );
            unlink($this->getRunningPath() . '/vios_base_startup-config.txt');
